...
import { Facts, ProjectHead, ProjectLinksRow, StackLine, Story } from '@/components/work/ProjectParts'
    ...
    mm.add(MQ.motion, () => {
      gsap.utils.toArray<HTMLElement>('.seq-msg', scope).forEach((row) => {
        ScrollTrigger.create({
          ...
          start: 'top 78%',
          once: true,
          onEnter: () => row.classList.add('is-drawn'),
        })
      })
    })
    ...
    <article ref={root} id="eduai" aria-labelledby="eduai-title" className="relative mt-40 scroll-mt-16">
      <div className="shell">
        <ProjectHead
          ...
          name={e.name}
          kicker={e.kicker}
          meta={[e.year, 'Solo build']}
          oneLiner={e.story[0].v as string}
        />
        ...
        <div className="grid12 mt-16 gap-y-14">
          <div className="col-span-4 md:col-span-12 lg:col-span-7">
            <p className="text-[clamp(1.5rem,2.8vw,2.5rem)] font-semibold leading-[1.15]">
              The interesting part wasn’t generating text. It was making{' '}
              <span className="text-signal">unreliable JSON</span> behave like an API contract.
            </p>
            <Story
              className="mt-12"
              rows={[
                { k: 'How it works', v: e.solution },
                { k: 'When it breaks', v: e.challenge },
              ]}
            />
          </div>
          <aside className="col-span-4 md:col-span-12 lg:col-span-4 lg:col-start-9" aria-label="Security">
            <p className="mono text-ink-muted">Behind the API</p>
            <ul className="mt-5 border-t border-line">
              {e.securityBlock.map((line, i) => (
                <li key={line} className="flex items-baseline justify-between gap-4 border-b border-line py-3">
                  <span
                    className={cx('font-semibold uppercase', i === 0 && 'text-measure')}
                    style={{ fontVariationSettings: "'wdth' 110" }}
                  >
                    {line}
                  </span>
                  {i === e.securityBlock.length - 1 ? <Metric source="eduaiRoutes">{endpoints}</Metric> : null}
                </li>
              ))}
            </ul>
          </aside>
        ...
        <div className="mt-16">
          <p className="mono text-ink-muted">From model output to a stored course</p>
          <Flow className="mt-6" horizontal nodes={e.flow} label="EduAI generation flow, in order" />
        </div>

        <DeepDive className="mt-16" summary="19 endpoints · the POST /api/courses sequence · security details · stack">
              ...
              <div className="mt-5 border border-line bg-bg">
                {e.api.map((group) => (
                  <table key={group.base} className="mono mono-sm w-full border-collapse">
                    <caption className="border-b border-line bg-bg-raised px-3 py-2 text-left text-ink">
                      {group.base}
                    </caption>
                    <tbody>
              ...
              <div className="relative mt-5 hidden md:block">
                <div className="grid grid-cols-5 gap-2">
                  {e.actors.map((a) => (
                    <p
                      key={a}
                      className="mono border border-line-strong bg-bg px-1.5 py-2 text-center text-[10px] leading-tight text-ink [overflow-wrap:anywhere]"
                    >
                      {a}
                    </p>
                  ))}
                      ...
                      return (
                        <li key={i} className="seq-msg relative h-[74px]">
                          ...
                          <span
                            aria-hidden="true"
                            className={cx(
                              'seq-arrow absolute top-[36px] h-px',
                              i === e.sequence.length - 1 ? 'bg-signal' : 'bg-ink',
                            )}
                            style={{
                              left: `${left}%`,
                              width: `${width}%`,
                          ...
                          >
                            <span
                              className={cx(
                                'absolute -top-[4px] h-0 w-0 border-y-[4px] border-y-transparent',
                                forward
                                  ? '-right-px border-l-[7px] border-l-current'
                                  : '-left-px border-r-[7px] border-r-current',
                                i === e.sequence.length - 1 ? 'text-signal' : 'text-ink',
                              )}
                            />
                          </span>
                          <p
